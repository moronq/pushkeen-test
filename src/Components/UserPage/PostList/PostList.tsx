        ...
        <section className={styles.posts}>
            <div className={styles.postsContainer}>
                ...
                <ul className={styles.postList}>
                    {userPosts.slice(0, countPost).map((el) => <PostItem key={el.id} title={el.title}
                                                                         userId={el.userId}
                                                                         postId={el.id}
                                                                         body={el.body}/>
                    )}
            ...
            <Button background={'black'} color='white' pdCol='15px' fontSize='11px' callBackFn={onAllPostButtonClick}>
                Смотреть все посты
            </Button>
        </section>
    );
