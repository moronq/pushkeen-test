...
const UserItemContainer = () => {
  ...
  return (<div className={styles.userItemListWrapper}>
    {portionNumber > 1 && <div className={styles.userItemPrevButton}>
      <Button borderRadius={'50%'} pdCol={'17px'} background={'black'}
        callBackFn={() => setPortionNumber(portionNumber - 1)}>
        <img src={arrow} alt='button-left'
          style={{transform: 'rotate(180deg)'}}/>
      </Button>
    </div>}
    <ul className={styles.usersItemContainer}>
      {usersList.filter((el, index) => index >= leftPortionItemNumber &&
          index < rightPortionItemNumber)
          .map((el) =>
