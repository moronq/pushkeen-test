...
const CommentsInput: React.FC<CommentsInputType> = ({postId}) => {
  ...
  const disabledButton = name === '' ? true : email === '' ? true :
      commentText === '';
  ...
  return (<div className={styles.commentInputWrapper}>
    <form className={styles.commentInputForm}>
      <div className={styles.commentInputContainer}>
        ...
        <input placeholder={'Иван Иванов'} type={'text'} id={'name'}
          value={name} onChange={(e) => setName(e.currentTarget.value)}/>
      </div>
      <div className={styles.commentInputContainer}>
        <label htmlFor="email">email:</label>
        <input placeholder={'IvanIvanov@gmail.com'} type={'email'} id={'email'}
      ...
      <div className={styles.commentInputContainer + ' ' + styles.commentInput}>
        ...
        <textarea className={styles.commentTextArea}
          placeholder={'Начните вводить комментарий...'} id={'comment'}
          value={commentText}
          onChange={(e) => setCommentText(e.currentTarget.value)}/>
      </div>
      <div className={styles.commentButtonContainer}>
        <Button disabled={disabledButton} height='35px' color='white'
          fontSize='11px' background={'black'} pdCol='20px'
