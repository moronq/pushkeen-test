...
import {fetchPostComments, fetchUserInfo, fetchUserPost, userSlice} from "../../store/Reducers/UserSlice";
...
const PostPage = () => {
    ...
    const {statusPost, userPosts, userPostComments, errorPost} = useAppSelector(state => state.userPage)
        ...
        contentPost = userPosts.length > 0 &&
                <>
                    <PostItem showAll={true}
                              title={userPosts.filter((el) => el.id.toString() === params.idPost)[0].title}
                              body={userPosts.filter((el) => el.id.toString() === params.idPost)[0].body}
                    ...
                    <ul className={styles.commentsList}>
                        {userPostComments.filter((el) => el.postId.toString() === params.idPost).map((el) => {
                            return <Comments key={el.id} body={el.body} email={el.email} name={el.name}/>
                        })}
                    </ul>
                </>
    ...
    return (
        <div className={styles.postPageContainer}>
            <div className={styles.postPageWrapper}>
                {contentPost}
                {!commentActive ? <Button callBackFn={()=>setCommentActive(true)} background={'black'} color={'white'} fontSize={'11px'} pdCol={'20px'}>
