...
import {GetUsersType, UserPostCommentsType,
  UserPostType} from '../../types/UsersTypes';
...
export const userSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
        ...
        .addCase(addPostComment.fulfilled,
            (state, action: any) => {
              state.userPostComments = state.userPostComments
...
export const fetchUserPost = createAsyncThunk('user/fetchUserPost',
    async (id: string) => {
      return await UsersAPI.getUserPost(id);
    });
export const fetchPostComments = createAsyncThunk('user/fetchPostComments',
    async (postId: string) => {
...
export const addPostComment = createAsyncThunk('user/addPostComment',
    async ({postId, bodyComment, name = 'user', email = 'user@gmail.com'}: {
        postId: string, bodyComment: string, name?:string,
        email?: string }) => {
      const comment = {
